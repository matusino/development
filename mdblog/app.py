...
from flask_wtf import FlaskForm#ztohto flask form sa bude dedit
...
@flask_app.route("/articles/<int:art_id>/", methods=["GET"])
    ...
    if article:
        ...
        if edit_form.validate():
            ...
            for error in login_form.errors:
                flash("{} is missing".format(error), "alert_danger")
            return redirect(url_for("view_login"))
...
def login_user():
    ...
    if login_form.validate():
        if login_form.username.data == flask_app.config["USERNAME"] and \
            ...
            flash("Wrong credentials", "alert_danger")
            return redirect(url_for("view_login"))
    else:
        for error in login_form.errors:
            flash("{} is missing".format(error), "alert_danger")
